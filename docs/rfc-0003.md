...
def encode_bytes_to_babel(data: bytes) -> str:
    ...
    # Marcador de versão do encoding: 'd' (enc-v4, utilizado pelo BSP v4, v5 e v6)
...
| enc-v4 | `'d'` | BSP v4, v5, v6 | **Atual** |

**Nota**: A versão do encoding (enc-v4, marcador `'d'`) é independente da versão do protocolo BSP. Os protocolos BSP v5 e v6 continuam utilizando o encoding enc-v4 sem alterações no esquema de codificação.
