...
A assinatura cobre todos os campos dos metadados, exceto o próprio `sig`:
*   `f` (nome do arquivo)
...
*   `chk` (array de chunks)
*   `mr`, `mh` (raiz e altura da árvore Merkle, quando presentes — BSP v6)

A partir do BSP v6, a raiz Merkle é calculada **antes** da assinatura, de modo
que a assinatura também protege a raiz. Ver [RFC 0007](rfc-0007.md).
...
*   [RFC 0007](rfc-0007.md) - Verificação por Árvore Merkle (a assinatura passa a cobrir a raiz)
